...
function tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest) {
  ...
  let maxSpeed = 0;
  redShirtSpeeds.sort((a, b) => a - b); // asc
  blueShirtSpeeds.sort((a, b) => b - a); // desc
  j = blueShirtSpeeds.length - 1;
  if (fastest) {
    ...
    for (i in blueShirtSpeeds) {
      if (redShirtSpeeds[i] > blueShirtSpeeds[j]) {
        maxSpeed += redShirtSpeeds[i];
      } else {
        maxSpeed += blueShirtSpeeds[j];
      }
      j = j - 1;
    }
  }
...
function tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest) {
  ...
  let j = blueShirtSpeeds.length - 1; // Initialize j to 0
